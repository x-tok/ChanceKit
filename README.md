...
4. 周视图以北京时间、周一为起点，支持上一周、下一周、日期跳转、类型与来源群筛选。跨天活动显示在对应日期；无法确认日期的通知列在“日期待确认”。
...
图片交给模型前由 sharp 解码、校正方向并转换成 JPEG；长海报按最多 1600×2200 像素分段，相邻分片重叠以保留跨边界文字。动图在预算内读取所有帧，只去掉像素完全相同的重复帧，再按顺序拼图，不再因为是动图就标为未完成。二维码在本机解码；网页明文网址及图片识别中明确出现的网址也计入报名链接来源，不允许活动模型凭空猜测链接。

每条消息最多读取 4 个页面、2 个文件、48 个网页/消息图片来源，最多 128 个图像分片，编码后总量 72 MB；不再到第 6 张就停止。网页/文件 5 MB、响应类型为图片的资源 20 MB、单资源 30 秒超时、最多 8 次跳转，整条消息最多 10 分钟。静态图输入上限约 2.68 亿像素，缩放后最多 3200 万像素；动图最多 240 帧、累计 8000 万像素。正文长度也有限制，超出真实安全上限仍会提示，不伪装成完整读取。公开图片下载有有界内存缓存，请求同一网站时限速；微信临时验证页只延迟重试一次，不绕过验证。DNS 在实际连接时校验并绑定，拒绝本机、内网、凭据 URL 和非标准端口，不携带浏览器 Cookie 或模型 API Key。公开材料下载直连，模型请求使用 Electron 系统网络设置。

群文件支持 DOCX 正文、内嵌图片和报名链接，以及 UTF-8 TXT、MD、CSV 文本。通过已连接 QQ 获取当前账号、已关注群归档消息中的附件，支持旧式 `/UUID` 文件 ID、下载 URL 和 Base64 数据。过期图片或旧文件 ID 会尝试重新读取原消息，并核对群号、发送者、时间和附件位置。托管 QQ 返回本地文件时，只读本应用私有 QQ 缓存内匹配文件名、大小的普通文件，拒绝越界路径和符号链接逃逸；不读取外接 NapCat 返回的任意本机路径。DOCX 禁止外部文件访问，解压最多 1000 项、20 MB。需要 QQ 的附件在离线时保留为“待检查”，并显示具体失败原因。

实际需要登录或验证码的页面、没有可提取静态正文/表单数据的动态网页、PDF/旧版 DOC 等其他文件、音视频、合并转发的展开内容暂不支持自动提取。资源失效、图像能力关闭或材料截断会记录为“待检查”，可单条或批量重试，不伪装成完整读取。更新读取器后，重启桌面应用并在“处理设置与记录”选择“重试未完成”；仅更新程序不会自动重新处理旧的待检查记录。
...
新增 SQLite 表：`activities`、`activity_sources`、`schedule_jobs`、`schedule_settings`。既有消息库自动增加内容版本字段，不删除消息或关注状态；活动、队列和处理开关与 QQ 账号隔离。
