...
3. 复制到本应用的 `QQRuntime.app`，在副本中生成一个小型加载入口并做 ad-hoc 签名。
...
5. 使用 QQ 自带 Electron 启动 NapCat，设置独立 `NAPCAT_WORKDIR`，不使用应用壳内的 Electron 加载 QQ 的 native wrapper。
...
平台真实验证应记录：操作系统与架构、QQ 核心版本、NapCat 版本、资料目录是否新建、二维码是否实际生成、账号确认、群列表、历史读取、实时消息和停止后进程退出。用户扫码之前只能确认“二维码链路”，不能记录成“真实消息采集通过”。
