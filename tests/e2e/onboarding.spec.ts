...
test('first-run onboarding completes QQ, DeepSeek and bounded group sync', async () => {
  ...
  try {
    ...
    await expect(page.getByRole('navigation', { name: '群聊分页' })).toHaveCount(0);
    const groupList = page.getByRole('group', { name: '选择关注群聊' });
    expect(await groupList.evaluate(element => element.scrollHeight > element.clientHeight)).toBe(true);
    await page.getByText('高校就业通知', { exact: true }).scrollIntoViewIfNeeded();
    await expect(page.getByText('高校就业通知', { exact: true })).toBeVisible();
    expect(await page.evaluate(() => document.documentElement.scrollHeight <= innerHeight)).toBe(true);
    await page.getByText('2027 届校园招聘信息交流', { exact: true }).scrollIntoViewIfNeeded();
    await page.getByText('2027 届校园招聘信息交流', { exact: true }).click();
    await groupList.evaluate(element => { element.scrollTop = 0; });
