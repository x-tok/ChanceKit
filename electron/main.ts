...
app.setName('群讯');
if (!app.isPackaged) app.setPath('userData', path.resolve('.dev-data'));
if (process.env.QUNXUN_TEST_DATA) app.setPath('userData', path.resolve(process.env.QUNXUN_TEST_DATA));
...
app.whenReady().then(async () => {
  ...
  worker = utilityProcess.fork(path.join(__dirname, 'worker.cjs'), [root, componentArchive], { serviceName: '群讯消息服务' });
  const ready = new Promise<void>(resolve => {
    worker.on('message', data => {
      if (data.ready) resolve();
      if (data.event && !window?.isDestroyed()) window?.webContents.send('qunxun:event', data.event);
  ...
  worker.on('exit', () => {
    ...
    pending.clear();
    if (!quitting && window) dialog.showErrorBox('消息服务已停止', '请重新打开群讯。已经归档的消息保留在本机。');
  });
  ...
  ipcMain.handle('qunxun:request', async (event, input) => {
    verifySender(event);
  ...
  ipcMain.handle('qunxun:connection', async event => {
    verifySender(event);
    try {
  ...
  ipcMain.handle('qunxun:choose-qq', async event => {
    verifySender(event);
  ...
  ipcMain.handle('qunxun:export', async (event, groupId) => {
    verifySender(event);
  ...
  ipcMain.handle('qunxun:open', async (event, input: string) => {
    verifySender(event);
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...(process.platform === 'darwin' ? [{ label: '群讯', submenu: [{ role: 'about' }, { type: 'separator' }, { role: 'quit' }] } as Electron.MenuItemConstructorOptions] : []),
    { label: '编辑', submenu: [{ role: 'undo' }, { role: 'redo' }, { type: 'separator' }, { role: 'cut' }, { role: 'copy' }, { role: 'paste' }, { role: 'selectAll' }] },
    { label: '窗口', submenu: [{ role: 'minimize' }, { role: 'close' }] },
  ]));
  window = new BrowserWindow({ width: 1240, height: 820, minWidth: 900, minHeight: 640, backgroundColor: '#ffffff', title: '群讯', autoHideMenuBar: true,
    webPreferences: { preload: path.join(__dirname, 'preload.cjs'), contextIsolation: true, nodeIntegration: false, sandbox: true },
  });
  window.webContents.setWindowOpenHandler(() => ({ action: 'deny' }));
  window.webContents.on('will-navigate', event => event.preventDefault());
  window.webContents.session.setPermissionRequestHandler((_webContents, _permission, callback) => callback(false));
  if (process.env.QUNXUN_RENDERER_URL) await window.loadURL(process.env.QUNXUN_RENDERER_URL);
