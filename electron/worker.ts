...
process.parentPort.on('message', async ({ data }: { data: any }) => {
  ...
  try {
    const value = data.command?.type === 'resolveAttachment' ? await service.resolveAttachment(data.command)
      : data.command?.type === 'resolveReply' ? await service.resolveReply(data.command) : await service.request(data.command);
