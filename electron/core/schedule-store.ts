...
import { addDays, emptyProcessingStatus, type Activity, type ActivityDetail, type ActivityInput, type ActivitySource, type ProcessingStatus, type SchedulePage, type ScheduleQuery } from '../../src/schedule';
...
export class ScheduleStore {
  ...
  claim(accountId: string, now = Date.now()): ProcessingJob | undefined {
    return this.transaction(() => {
      ...
      if (!row) return;
      this.db.prepare("UPDATE schedule_jobs SET status='running',attempts=attempts+1,updated_at=? WHERE message_key=?").run(now, row.message_key);
      ...
      return { message, groupName: String(row.name), hash: String(row.input_hash), attempts: Number(row.attempts) + 1, context };
    });
  }
  ...
  fail(job: ProcessingJob, error: string, now = Date.now()) {
    this.db.prepare(`UPDATE schedule_jobs SET status=?,error=?,available_at=?,updated_at=?
  ...
  complete(job: ProcessingJob, result: ExtractionResult): boolean {
    const { activities } = extractionSchema.parse({ activities: result.activities });
    return this.transaction(() => {
      if (!this.isCurrent(job)) return false;
      this.db.prepare('DELETE FROM activity_sources WHERE message_key=?').run(job.message.key);
      for (const activity of activities) {
        ...
        this.db.prepare('INSERT OR REPLACE INTO activity_sources VALUES(?,?,?)')
          .run(id, job.message.key, JSON.stringify({ activity, materials: result.materials, warnings: result.warnings }));
      }
      ...
      this.db.prepare("UPDATE schedule_jobs SET status=?,error=?,updated_at=? WHERE message_key=?")
        .run(result.warnings.length ? 'partial' : 'completed', result.warnings.join('；').slice(0, 800), Date.now(), job.message.key);
      return true;
    });
  }
  ...
  detail(accountId: string, id: string, groupId?: string): ActivityDetail | null {
    ...
    return {
      activity: { ...payload.activity, id, updatedAt: Number(rows[0].updated_at), sourceCount: sources.length,
        ...
        needsReview: !payload.activity.startDate || !payload.activity.startTime || sources.some(source => source.warnings.length > 0) },
      sources,
    };
  }
