...
export interface DownloadedMaterial { url: string; contentType: string; bytes: Uint8Array }
...
export async function downloadPublicMaterial(input: string, parentSignal: AbortSignal, request = httpFetch): Promise<DownloadedMaterial> {
  ...
  try {
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      ...
      if (request === httpFetch) {
        ...
        hostSchedule.set(url.host, next + 350);
      ...
      const material = { url: url.href, contentType, bytes: Buffer.concat(chunks) };
...
export async function collectMessageMaterials(
  ...
  const addImage = async (bytes: Uint8Array) => {
    if (!imageInput || result.images.length >= MAX_IMAGES) {
      result.warnings.push('图片未发送：当前模型不支持图像或已达到图片数量上限。'); return;
    }
    const normalized = await normalizeMaterialImage(bytes, MAX_IMAGES - result.images.length, signal);
    signal.throwIfAborted();
    ...
    result.imageGroups.push({ label: `来源图片 ${result.imageGroups.length + 1}`, images: normalized.images });
    if (normalized.truncated) result.warnings.push('图片超过 128 个分片或动图解码上限，部分内容未读取。');
  ...
  for (const segment of message.segments) if (segment.type === 'text') linksInText(String(segment.data.text ?? ''));
  for (const [index, segment] of message.segments.entries()) {
    if (segment.type === 'image') {
      const value = segment.data.url ?? segment.data.file;
      if (typeof value === 'string' && /^https?:\/\//.test(value)) {
        addURL(value, images);
        try { imageSegments.set(publicMaterialUrl(value).href, index); } catch {}
    ...
    if (segment.type === 'json') {
      try {
        ...
        const walk = (value: unknown, depth = 0) => {
          if (depth > 6) return;
          if (typeof value === 'string') { linksInText(value); return; }
  ...
  for (const file of files.slice(0, 2)) {
    signal.throwIfAborted();
    if (!/\.(docx|txt|md|csv)$/i.test(file.name)) { result.warnings.push('暂不支持此文件格式，可读取 DOCX、TXT、MD 和 CSV。'); continue; }
    try {
      ...
      for (const image of document.images) {
        try { await addImage(image); }
        catch { signal.throwIfAborted(); result.warnings.push('文件中部分图片格式损坏，未发送给模型。'); }
      }
    } catch (error) {
      signal.throwIfAborted();
      const detail = error instanceof Error && /^(请|附件|文件|DOCX|QQ|NapCat|连接|此消息|当前|内容)/.test(error.message)
  ...
  for (const url of [...pages].slice(0, MAX_PAGES)) {
    ...
    try {
      ...
      const text = new TextDecoder(charset).decode(data.bytes);
      ...
      if (page.restricted && new URL(url).hostname === 'mp.weixin.qq.com') {
        ...
        if (/text\/html|application\/xhtml\+xml/i.test(data.contentType)) page = parseMaterialPage(new TextDecoder(charset).decode(data.bytes), data.url);
      }
      if (page.restricted) {
        result.warnings.push(`网站返回了验证或登录页面，未读取正文：${new URL(url).hostname}。`);
        continue;
      }
      if (page.text.length < 30 && !page.images.length) result.warnings.push('部分网页未返回足够正文，需核对原页面。');
      ...
      result.materials.push({ url, kind: 'page', title: page.title.slice(0, 200) });
  ...
  if (images.size > MAX_IMAGE_SOURCES) result.warnings.push(`图片来源超过 ${MAX_IMAGE_SOURCES} 张，只读取前 ${MAX_IMAGE_SOURCES} 张。`);
  for (const url of [...images].slice(0, MAX_IMAGE_SOURCES)) {
