...
Call submit_daily_activities exactly once. Always include both activities and information arrays; use an empty array when a category has no results.
...
For recruiting announcements that contain useful jobs, application guidance or employer information but no scheduled event, add one concise information item tied to its sourceRef. Do not add ordinary chat or a source already represented by an activity.
