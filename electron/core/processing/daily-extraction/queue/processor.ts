...
type DailyRunner = (job: DailyProcessingJob, settings: StoredModelSettings, signal: AbortSignal) => Promise<DailyExtractionResult>;

export class DailyScheduleProcessor {
  private accountId = '';
  ...
  private published = '';
  private running = new Map<string, { job: DailyProcessingJob; controller: AbortController; promise: Promise<void> }>();
  private timer: NodeJS.Timeout;

  constructor(
    readonly store: DailyScheduleStore,
    private loadSettings: () => Promise<StoredModelSettings | null>,
    private run: DailyRunner,
    private emit: () => void,
  ) {
  ...
  refresh() {
    ...
    for (const active of this.running.values()) if (!this.store.isCurrent(active.job)) active.controller.abort();
    this.publish();
    this.wake();
  }
